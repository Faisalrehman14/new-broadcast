...
describe('campaign helpers', () => {
  it('maps speed presets to delay', () => {
    assert.equal(speedToDelayMs('safe').delayMs, 800);
    assert.equal(speedToDelayMs('balanced').delayMs, 400);
    assert.equal(speedToDelayMs('fast').delayMs, 250);
    assert.equal(speedToDelayMs('turbo').delayMs, 150);
    assert.equal(speedToDelayMs('custom', 250).delayMs, 250);
    assert.equal(speedToDelayMs('custom', 50).delayMs, 100);
  });
